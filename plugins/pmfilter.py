...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        shortlink= await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.get('file_id', file.get('_id'))}")
        if not settings["button"]:
            cap+="\n\n<b>📚 <u>Your Requested Files</u> 👇\n\n</b>"
            for file in files:
                cap += f"<b>\n<a href='{shortlink}'> 📁 {get_size(file.file_size)} ▷ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        shortlink= await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.get('file_id', file.get('_id'))}")
        if settings["button"]:
            ...
            for file in files:
                cap += f"<b><a href='{shortlink}'> 📁 {get_size(file.file_size)} ▷ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b>"
